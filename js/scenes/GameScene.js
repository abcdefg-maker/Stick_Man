...
class GameScene extends Phaser.Scene {
    ...
    onGameOver() {
        ...
        if (isNewHighScore) {
            ...
            EventManager.emit(GameEvents.HIGH_SCORE_UPDATED, this.score);
        }

        EventManager.emit(GameEvents.GAME_OVER, { score: this.score, isNewHighScore });

        // 显示游戏结束UI
        this.showGameOverUI(isNewHighScore);

        // 延迟后重新开始
        this.time.delayedCall(GameplayConfig.GAME_FLOW.GAME_OVER_DELAY, () => {
            this.scene.start('MenuScene');
        });
    }

    showGameOverUI(isNewHighScore) {
        const canvas = LayoutConfig.CANVAS;
        ...
        const finalScoreText = this.add.text(
            finalScoreConfig.x,
            finalScoreConfig.y,
            `${finalScoreConfig.prefix}${this.score}`,
            {
